...
export default class Level3a extends MainGame {
    ...
    create ()
    {
        ...
        this.timerText = this.add.text(20, 0, `Color  NOT=`, fontStyle2);     
        this.colorrect = this.add.rectangle(150, 20, 30, 30, 0x008000)
    ...
    arrangeGrid ()
    {
        ...
        children[a].setFrame('greenbanana.png')
        ...
        for (let i = 0; i < 16; i++)
        {
            if (i != a && i != b) {
                children[i].setFrame(this.game.config.colors[Math.floor((5*Math.random()))] + this.game.config.fruits[Math.floor((4*Math.random()))] +'.png')
                    if (children[i].frame.customData.color != 'green' || children[i].frame.customData.fruit == 'banana')
